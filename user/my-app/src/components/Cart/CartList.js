...
class CartList extends Component {
    ...
    componentDidMount() {
        window.scroll(0,0)
        axios.get(ApiURL.CartList(LocalStorageHelper.getEmail('email'))).then(response=> {
    ...
    confirmOnClick=()=>{
        ...
        if(city.length===0){
            ...
            MyFormData.append('email',LocalStorageHelper.getEmail('email'))
    ...
    render() {
        ...
        let totalPriceSum=0;
        const MyView=MyList.map((CarttList,i)=>{
            totalPriceSum=totalPriceSum+parseInt(CarttList.total_price)
            return <div className="container">
                    <div className="row">
                        <div className="col-md-3 text-center col-lg-3 col-sm-4 col-6">
                            <img className="w-100" src={CarttList.img} alt=""/>
                            <button onClick={()=>this.removeItem(CarttList.id)}  className="btn mt-2 btn-sm site-btn"><i className="fa fa-trash-alt"/></button>
                            <button onClick={()=>this.itemPlus(CarttList.id,CarttList.product_quantity,CarttList.unit_price)}  className="btn mt-2 mx-1 btn-sm site-btn"><i className="fa fa-plus"/></button>
                            <button onClick={()=>this.itemMinus(CarttList.id,CarttList.product_quantity,CarttList.unit_price)}  pquantity={CarttList.product_quantity} className="btn mt-2 mx-1 btn-sm site-btn"><i className="fa fa-minus"/></button>
                        </div>
                        <div className="col-md-7 col-lg-7 col-sm-8 col-6">
                            <h5 className="product-name-on-card">{(CarttList.product_name).substring(0,50) }</h5>
                            <h5 className="product-price-on-card">Total Price:{CarttList.total_price}TK</h5>
                            <h5 className="product-name-on-card">{(CarttList.product_info)}</h5>
                            <h5 className="product-price-on-card">Quantity:{ CarttList.product_quantity}</h5>
                        </div>
                    </div>
                    <hr/>
                   </div>
